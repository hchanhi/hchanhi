# Hi, I'm Chanhee 👋

Backend developer with a deep interest in payment systems.

I operate a PG (Payment Gateway) service, solving real production problems —
payment integrations, APIs, incident and error analysis, and system monitoring.

### 🛠️ Interested in

`Backend` · `API` · `Payment` · `Monitoring` · `Automation` · `Database`

I like spotting small inconveniences and fixing them myself.
I automate repetitive work, and I care about understanding problems from their root cause.

<img src="https://github-profile-summary-cards.vercel.app/api/cards/stats?username=hchanhi&theme=github_dark"
     alt="GitHub stats" width="450">
